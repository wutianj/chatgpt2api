...
### Gemini Business Provider（可选）

Gemini Business 作为独立 Provider 接入，账号池由外部 Gemini Business 兼容网关维护，不与本项目的 GPT 账号池混用。管理员登录控制台后，在「系统设置 → 外部服务 → Gemini Business Provider」中填写：

- 网关地址，例如 `http://gemini-business2api:7860` 或其 `/v1` 地址
- 网关密钥（如果网关启用了 Bearer 鉴权）
- 对话模型和生图模型，每行一个；模型名必须使用 `gemini-` 前缀
- 请求超时时间

启用并配置地址后，模型会自动出现在现有用户端的模型选择器和 `/v1/models` 中。当前复用现有页面与计费链路，支持 `/v1/chat/completions`、`/v1/images/generations`、`/v1/images/edits` 及用户端图片任务接口；未配置时不会影响 GPT 功能。
