...
TerminalTaskHook = Callable[[dict[str, Any], dict[str, object]], None]
...
class EditableFileTaskService:
    ...
    def submit_ppt(
        ...
        *,
        ...
        base_url: str = "",
        on_terminal: TerminalTaskHook | None = None,
    ) -> dict[str, Any]:
        return self._submit(
            identity,
    ...
    def _submit(
        ...
        *,
        ...
        threading.Thread(
            ...
            args=(key, kind, prompt, base64_images, dict(identity), base_url, on_terminal),
            name=f"{kind}-file-task-{task_id[:16]}",
            daemon=True,
        ).start()
        return _public_task(task)
    ...
    def _run_task(
        ...
        identity: dict[str, object],
        base_url: str,
        on_terminal: TerminalTaskHook | None,
    ) -> None:
        started = time.time()
        token = ""
        account_email = ""
        ...
        try:
            ...
            self._notify_terminal(key, identity, on_terminal)
    ...
    def _notify_terminal(
        ...
        with self._lock:
            owner_id, task_id = key.rsplit(":", 1)
            task = self._repository.get(owner_id, task_id)
        if task is None:
            return
        try:
            on_terminal(task, identity)
        except Exception as exc:
            logger.error({
                "event": "editable_file_task_terminal_hook_failed",
                "task_id": task_id,
                "error": str(exc),
            })
