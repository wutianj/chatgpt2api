...
def _account_export_meta(account: dict, credentials: dict, extra: dict) -> dict[str, str]:
    """Return the Sub2API OAuth identity fields needed by the local Codex pool."""
    ...
    sources = (credentials, extra, account)
    aliases = {
    ...
    for target, keys in aliases.items():
        for source in sources:
            value = next((_clean(source.get(key)) for key in keys if _clean(source.get(key))), "")
            if value:
...
def _fetch_access_token_from_export(server: dict, account_id: str) -> tuple[str, dict]:
    ...
    return access_token, _account_export_meta(account, credentials, extra)
...
def _fetch_access_tokens_for_accounts(server: dict, account_ids: list[str]) -> tuple[dict[str, tuple[str, dict]], dict[str, str]]:
    ...
    for index, account in enumerate(accounts):
        ...
        results[account_id] = (
            access_token,
            _account_export_meta(account, credentials, extra),
        )
    ...
    def _run_import(
        ...
        *,
        ...
        def append_payload(account_id: str, token: str, meta: dict) -> None:
            ...
            if "refresh_token" in payload and not payload.get("client_id"):
                payload["client_id"] = codex_oauth_client_id
            for metadata_key in ("account_id", "user_id"):
                metadata_value = _clean(meta.get(metadata_key)) if isinstance(meta, dict) else ""
                if metadata_value:
