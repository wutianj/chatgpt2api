...
from services.gemini_provider import configured_models as configured_gemini_models
...
class ModelCatalogService:
    ...
    def view(self) -> ModelCatalogView:
        ...
        chat_models = _unique([
            *(configured_chat_models or list(FALLBACK_CHAT_MODELS)),
            *configured_gemini_models("chat"),
        ])

        if configured_image_models:
            image_source = "config"
            image_models = _unique([
                *configured_image_models,
                *configured_gemini_models("image"),
            ])
        else:
            ...
            image_models = _unique([
                *(account_models or list(FALLBACK_IMAGE_MODELS)),
                *configured_gemini_models("image"),
            ])
