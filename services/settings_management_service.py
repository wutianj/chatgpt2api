...
_FIELD_SPECS: dict[str, dict[str, Any]] = {
    ...
    "gemini_provider.enabled": _field_metadata(False),
    "gemini_provider.base_url": _field_metadata(""),
    "gemini_provider.api_key": _field_metadata("", sensitive=True),
    "gemini_provider.chat_models": _field_metadata([]),
    "gemini_provider.image_models": _field_metadata([]),
    "gemini_provider.timeout_secs": _field_metadata(180, min=5, max=600, unit="seconds"),
...
class SettingsManagementService:
    ...
    def _project_settings(self, effective: dict[str, Any], stored: dict[str, Any]) -> SettingsValues:
        ...
        gemini_provider = _dict(effective.get("gemini_provider"))
        stored_gemini_provider = _dict(stored.get("gemini_provider"))
        ...
        return SettingsValues(
            ...
            gemini_provider=GeminiProviderSettings(
                enabled=_bool(gemini_provider.get("enabled"), False),
                base_url=_url(gemini_provider.get("base_url")),
                api_key="",
                has_api_key=bool(_text(stored_gemini_provider.get("api_key"))),
                chat_models=_string_list(gemini_provider.get("chat_models")),
                image_models=_string_list(gemini_provider.get("image_models")),
                timeout_secs=max(5, min(600, int(gemini_provider.get("timeout_secs") or DEFAULT_GEMINI_PROVIDER["timeout_secs"]))),
            ),
