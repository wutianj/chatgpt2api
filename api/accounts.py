...
from services.account_credentials import decode_access_token_payload
...
class AccountCredentialImportRequest(BaseModel):
    records: list[Any] = Field(default_factory=list)
    sync_after_import: bool = True
    return_items: bool = False
    target_group_id: str | None = None
...
def _record_credential_text(record: dict[str, Any], *keys: str) -> str:
    containers = [
        record.get("credentials") if isinstance(record.get("credentials"), dict) else {},
        record.get("credential") if isinstance(record.get("credential"), dict) else {},
        record.get("tokens") if isinstance(record.get("tokens"), dict) else {},
        record.get("auth") if isinstance(record.get("auth"), dict) else {},
...
def _token_claim_fields(access_token: str, id_token: str) -> dict[str, str]:
    access_payload = decode_access_token_payload(access_token)
    id_payload = decode_access_token_payload(id_token)
    ...
    return {
        ...
        "account_id": _clean_text(
            ...
            or id_payload.get("chatgpt_account_id")
        ),
        "user_id": _clean_text(
            auth_claim.get("chatgpt_user_id")
            or access_payload.get("chatgpt_user_id")
        ...
        "client_id": _clean_text(access_payload.get("client_id")),
    }


def _normalize_account_credential_record(
    record: dict[str, Any],
    target_group_id: str | None,
) -> tuple[dict[str, str], dict[str, Any]]:
    credentials = extract_import_credentials(record)
    access_token = _clean_text(credentials.get("access_token"))
    refresh_token = _clean_text(credentials.get("refresh_token"))
    id_token = _clean_text(credentials.get("id_token"))
    claim_fields = _token_claim_fields(access_token, id_token)

    management_id = _record_credential_text(
        record,
        "management_id",
        "managementId",
        "local_account_id",
    ...
    previous_access_token = _record_credential_text(
        record,
    ...
    account_id = (
        ...
        or _clean_text(credentials.get("chatgpt_account_id"))
        or claim_fields["account_id"]
    )
    user_id = (
        _record_credential_text(record, "user_id", "userId", "chatgpt_user_id", "chatgptUserId")
        or _clean_text(credentials.get("chatgpt_user_id"))
        or claim_fields["user_id"]
    )

    client_id = _clean_text(credentials.get("client_id")) or claim_fields["client_id"]
    source_type = _record_credential_text(record, "source_type", "sourceType")
    export_type = _record_credential_text(record, "export_type", "exportType")
    raw_type = _record_credential_text(record, "type", "account_type", "accountType")
    auth_mode = _clean_text(credentials.get("auth_mode")) or _record_credential_text(record, "auth_mode", "authMode")
    plan_type = (
        _record_credential_text(
            record,
            "plan_type",
        ...
        or claim_fields["plan_type"]
    )
    if raw_type and raw_type.strip().lower() not in {"codex", "oauth", "openai"}:
        plan_type = raw_type
    ...
    inferred_codex = (
        _clean_text(client_id) == codex_oauth_client_id
        or export_type.strip().lower() == "codex"
        or raw_type.strip().lower() == "codex"
    )
    if source_type:
        updates["source_type"] = source_type
    elif inferred_codex:
        updates["source_type"] = "codex"
        if client_id:
            updates["client_id"] = client_id
        elif refresh_token:
            updates["client_id"] = codex_oauth_client_id

    if target_group_id is not None:
        updates["group_id"] = target_group_id
    else:
        group_id = _record_credential_text(record, "group_id", "groupId", "account_group_id", "accountGroupId")
        if group_id:
            updates["group_id"] = group_id

    identity = {
        "management_id": _clean_text(management_id).lower(),
        "previous_access_token": previous_access_token,
        "access_token": access_token,
        "account_id": _clean_text(account_id).lower(),
        "user_id": _clean_text(user_id).lower(),
        "email": _clean_text(email).lower(),
    }
    return identity, updates


def _credential_target_indexes() -> dict[str, dict[str, list[dict[str, Any]]]]:
    ...
    def add(index_name: str, key: object, account: dict[str, Any]) -> None:
        normalized = _clean_text(key).lower()
        if not normalized:
            return
        bucket = indexes[index_name].setdefault(normalized, [])
        if not any(_clean_text(item.get("management_id")) == _clean_text(account.get("management_id")) for item in bucket):
            bucket.append(account)

    for account in account_service.list_accounts():
        add("management_id", account.get("management_id"), account)
        add("account_id", account.get("account_id"), account)
        add("user_id", account.get("user_id"), account)
        add("email", account.get("email"), account)
    return indexes


def _credential_target_label(identity: dict[str, str], row_number: int) -> str:
    for key in ("management_id", "account_id", "user_id", "email"):
...
def _resolve_credential_import_target(
    ...
    def add_matches(items: list[dict[str, Any]]) -> None:
        for item in items:
            management_id = _clean_text(item.get("management_id")).lower()
            if not management_id or management_id in seen_ids:
                continue
            seen_ids.add(management_id)
            matches.append(item)

    previous_token = _clean_text(identity.get("previous_access_token"))
    if previous_token:
        account = _get_account_by_token_identity(previous_token)
        if account is not None:
            add_matches([account])

    current_token = _clean_text(identity.get("access_token"))
    if current_token:
        account = _get_account_by_token_identity(current_token)
        if account is not None:
            add_matches([account])

    for key in ("management_id", "account_id", "user_id", "email"):
        value = _clean_text(identity.get(key)).lower()
        if value:
            add_matches(indexes[key].get(value, []))
...
def create_router() -> APIRouter:
    ...
    @router.post("/api/accounts/import/credentials")
    async def import_account_credentials(
            body: AccountCredentialImportRequest,
            authorization: str | None = Header(default=None),
    ):
        """Patch existing accounts with freshly logged-in AT/RT credential bundles."""
        require_admin(authorization)
        target_group_id = _target_account_group_id(body.target_group_id)
        indexes = _credential_target_indexes()
        stats = {
        ...
        for row_index, record in enumerate(body.records, start=1):
            if not isinstance(record, dict):
                stats["invalid"] += 1
                errors.append({
                    "id": f"row-{row_index}",
                    "code": "invalid_record",
                    "message": "记录不是 JSON 对象",
                })
                continue

            identity, updates = _normalize_account_credential_record(record, target_group_id)
            sensitive_values.extend(
                [
                    value for value in (
                        updates.get("access_token"),
                        updates.get("refresh_token"),
                        updates.get("id_token"),
                    )
                    if _clean_text(value)
                ]
            )
            if record.get("proxy"):
                proxy_values.append(_clean_text(record.get("proxy")))

            has_credentials = any(
                _clean_text(updates.get(key))
                for key in ("access_token", "refresh_token", "id_token")
            )
            label = _credential_target_label(identity, row_index)
            if not has_credentials:
                stats["missing_credentials"] += 1
                errors.append({
                    "id": label,
                    "code": "missing_credentials",
                    "message": "没有找到可更新的 AT/RT/ID Token",
                })
                continue

            current, target_error = _resolve_credential_import_target(identity, indexes)
            if current is None:
                if target_error == "ambiguous_target":
                    stats["ambiguous_target"] += 1
                    message = "匹配到多个账号，请补充 management_id 后重试"
                else:
                    stats["missing_target"] += 1
                    message = "没有匹配到已有账号，请提供 management_id、account_id、user_id 或邮箱"
                errors.append({
                    "id": label,
                    "code": target_error or "missing_target",
                    "message": message,
                })
                continue

            account_id = _clean_text(current.get("management_id")).lower()
            current_token = _clean_text(current.get("access_token"))
            if not account_id or not current_token:
                stats["missing_target"] += 1
                errors.append({
                    "id": label,
                    "code": "missing_target",
                    "message": "匹配账号缺少本地账号 ID 或当前 AT",
                })
                continue
            labels[account_id] = _clean_text(current.get("email")) or label or account_id
            ...
            stats["accepted"] += 1
            updated_id = _clean_text(updated.get("management_id")).lower() or account_id
            updated_token = _clean_text(updated.get("access_token"))
            updated_ids.append(updated_id)
            if updated_token and updated_id:
                targets.append((updated_token, updated_id))
            labels[updated_id] = _clean_text(updated.get("email")) or labels.get(account_id, updated_id)
        ...
        if body.sync_after_import and targets:
            ...
            errors.extend(sanitized_errors)

        updated_ids, removed_ids = _partition_account_ids(updated_ids)
        return {
            **_account_mutation_response(
                added=0,
                skipped=(
                    stats["invalid"]
                    + stats["missing_credentials"]
                ...
                errors=errors,
                events=_account_mutation_events(
                    action="import_account_credentials",
                    success_ids=updated_ids,
                    removed_ids=removed_ids,
                    errors=errors,
                    labels=labels,
                    success_message="账号凭据已修复",
                    removed_message="账号凭据修复后已被移除",
                ),
                include_items=body.return_items,
            ),
            **stats,
    ...
    async def finish_oauth_login(
        ...
        client_id = _clean_text(tokens.get("client_id"))
        if client_id:
            payload["client_id"] = client_id
        target_group_id = _target_account_group_id(body.target_group_id)
        if target_group_id is not None:
            payload["group_id"] = target_group_id
        identity, updates = _normalize_account_credential_record(payload, target_group_id)
        explicit_account_id = _clean_text(body.account_id).lower()
        if explicit_account_id:
            identity["management_id"] = explicit_account_id
        current, target_error = _resolve_credential_import_target(
            identity,
            _credential_target_indexes(),
        )
        if explicit_account_id and current is None:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if current is None and target_error == "ambiguous_target":
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "matched multiple accounts; provide account_id",
                    "code": "ambiguous_target",
                },
            )
        if current is not None:
            current_token = _clean_text(current.get("access_token"))
            account_id = _clean_text(current.get("management_id"))
            if not current_token or not account_id:
                raise HTTPException(status_code=500, detail={"error": "matched account is incomplete"})
            try:
            ...
            if account is None:
                raise HTTPException(status_code=500, detail={"error": "matched account could not be updated"})
            updated_id = _clean_text(account.get("management_id")) or account_id
            labels = {
                updated_id: (
                    _clean_text(account.get("email"))
                    or _clean_text(current.get("email"))
                    or updated_id
                )
            }
            return _account_mutation_response(
                added=0,
                skipped=0,
                synced=0,
                ...
                events=_account_mutation_events(
                    action="oauth_login",
                    success_ids=[updated_id],
                    labels=labels,
                    success_message="OAuth 凭据已更新",
                ),
                include_items=False,
            )
