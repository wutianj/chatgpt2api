...
export const accountImportsApi = {
  ...
  finishOAuthLogin: (
    ...
    apiClient.post<
      { session_id: string; callback: string; account_id: string; target_group_id: string | null },
      AccountMutationResponse
    >(
      '/api/accounts/oauth/finish',
      { session_id: sessionId, callback, account_id: accountId, target_group_id: targetGroupId },
    ),
