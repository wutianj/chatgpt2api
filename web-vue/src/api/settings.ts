...
export function buildThirdPartyHref(appUrl: string, baseUrl: string, apiKey: string) {
  const url = appUrl.trim()
  try {
    const target = new URL(url)
    target.searchParams.set('apiKey', apiKey)
    target.searchParams.set('baseUrl', baseUrl)
    const bootstrap = new URLSearchParams(target.hash.replace(/^#/, ''))
    bootstrap.set('access_token', apiKey)
    target.hash = bootstrap.toString()
    return target.toString()
  } catch {
    const separator = url.includes('?') ? '&' : '?'
    return `${url}${separator}apiKey=${encodeURIComponent(apiKey)}&baseUrl=${encodeURIComponent(baseUrl)}#access_token=${encodeURIComponent(apiKey)}`
  }
}
...
export const settingsApi = {
  ...
  createCanvasSession: () =>
    apiClient.post<Record<string, never>, CanvasSessionView>('/api/integrations/infinite-canvas/session', {}),
