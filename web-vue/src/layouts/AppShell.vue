      ...
      >
        <div class="flex h-16 items-center px-5 pt-4 lg:h-20 lg:pt-5">
          <div class="flex min-w-0 items-center">
            <div class="shell-sidebar-brand shrink-0 text-foreground" aria-hidden="true">
              <svg
                aria-hidden="true"
                viewBox="0 0 24 24"
                class="h-6 w-6"
                fill="currentColor"
              >
                <path d="M12 2.5 14.25 9l6.25 3-6.25 3L12 21.5 9.75 15 3.5 12l6.25-3L12 2.5Zm0 5.8L11 11l-2.75 1L11 13l1 2.7 1-2.7 2.75-1L13 11l-1-2.7Z" />
              </svg>
            </div>
...
type NavigationItem = {
  ...
  label: string
  icon: string
  capability: AuthCapability | 'user'
}
...
const menuItems: NavigationItem[] = [
  {
    path: '/',
    label: '工作台',
    icon: 'M4 5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-5l-3 3v-3H6a2 2 0 0 1-2-2V5zm4 3h8V6H8v2zm0 4h5v-2H8v2z',
    capability: 'user',
  },
  {
    path: '/chat',
    label: 'AI 对话',
    icon: 'M5 4h14a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-5l-4 4v-4H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm1 3v6h12V7H6z',
    capability: 'user',
  },
  {
    path: '/image',
    label: 'AI 生图',
    icon: 'M4 5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5zm3 2v10h10V7H7zm2 7 2.2-2.7 1.7 2 1.5-1.8L17 14H9z',
    capability: 'user',
  },
  {
    path: '/canvas',
    label: '无限画布',
    icon: 'M4 5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5zm4 3h8v8H8V8zm2 2v4h4v-4h-4z',
    capability: 'user',
  },
  {
    path: '/tasks',
    label: '我的任务',
    icon: 'M6 3h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2zm2 4v2h8V7H8zm0 4v2h8v-2H8zm0 4v2h5v-2H8z',
    capability: 'user',
  },
  {
    path: '/wallet',
    label: '余额与套餐',
    icon: 'M4 6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v2h-3a3 3 0 0 0 0 6h3v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6zm14 4h3v2h-3a1 1 0 1 1 0-2z',
    capability: 'user',
  },
  {
    path: '/account',
  ...
  {
    path: '/admin/gemini-accounts',
    label: 'Gemini 账号池',
    icon: 'M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18zm-1 4h2v5h-2V7zm0 7h2v2h-2v-2z',
    capability: 'admin_console',
  },
  {
    path: '/admin/users',
    label: '用户管理',
    icon: 'M16 11a3 3 0 1 0-2.9-3.7A4 4 0 1 0 8 14h8a3 3 0 0 0 0-6zm-8 1a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm8 0h-2a5.8 5.8 0 0 0-.8-2h2.8a1 1 0 1 1 0 2zM5 19a3 3 0 0 1 6 0H5z',
    capability: 'admin_console',
  },
  {
    path: '/admin/orders',
    label: '套餐与兑换码',
    icon: 'M4 5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5zm3 2v2h10V7H7zm0 4v2h6v-2H7zm0 4v2h8v-2H7z',
    capability: 'admin_console',
  },
  {
    path: '/admin/audit',
    label: '操作审计',
    icon: 'M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm3 4v2h8V8H8zm0 4v2h8v-2H8zm0 4v2h5v-2H8z',
    capability: 'admin_console',
  },
  {
    path: '/admin/logs',
...
const visibleMenuItems = computed(() => {
  return menuItems.filter(item => item.capability === 'user' || authStore.hasCapability(item.capability))
})
...
function isRoutePathActive(path: string, name: string, currentPath: string) {
  ...
  if (!normalized) return name === 'home' || currentPath === '/'
  if (path === '/admin') return name === 'admin-dashboard' || currentPath === '/admin'
...
let canvasSessionGeneration = 0

async function refreshCanvasHref() {
  const generation = ++canvasSessionGeneration
  canvasHref.value = ''
  await loadPublicRuntimeConfig()
  const canvas = thirdPartyApps.value?.infinite_canvas
  if (!authStore.isLoggedIn || !canvas?.enabled || !canvas.url.trim()) return
  try {
    const session = await settingsApi.createCanvasSession()
    if (generation !== canvasSessionGeneration) return
    canvasHref.value = buildThirdPartyHref(canvas.url, apiBaseUrl.value, session.access_token)
  } catch {
    if (generation === canvasSessionGeneration) canvasHref.value = ''
  }
}
...
const routeViewLoaders: Record<string, () => Promise<unknown>> = {
  ...
  '/chat': () => import('@/views/Studio.vue'),
  '/image': () => import('@/views/Studio.vue'),
  '/canvas': () => import('@/views/UserCanvas.vue'),
  '/tasks': () => import('@/views/UserTasks.vue'),
  '/account': () => import('@/views/UserAccount.vue'),
  '/wallet': () => import('@/views/UserWallet.vue'),
  '/admin': () => import('@/views/Dashboard.vue'),
  '/admin/accounts': () => import('@/views/Accounts.vue'),
  '/admin/gemini-accounts': () => import('@/views/GeminiAccounts.vue'),
  '/admin/users': () => import('@/views/AdminUsers.vue'),
  '/admin/orders': () => import('@/views/AdminOrders.vue'),
  '/admin/audit': () => import('@/views/AdminAudit.vue'),
  '/admin/logs': () => import('@/views/Logs.vue'),
  '/admin/gallery': () => import('@/views/Gallery.vue'),
  '/admin/monitor': () => import('@/views/Monitor.vue'),
  '/admin/proxy': () => import('@/views/Proxy.vue'),
...
async function openInfiniteCanvas() {
  await refreshCanvasHref()
  if (!canvasHref.value) return
  const ok = await confirmDialog.ask({
    title: '打开无限画布',
      message: '即将打开外部画布，并附带当前接口地址和短期画布访问令牌。是否继续？',
    confirmText: '打开',
    cancelText: '取消',
  })
  if (ok) {
