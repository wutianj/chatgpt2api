  ...
  <div class="space-y-6">
    <PagePanel v-if="localSettings" class="settings-page-panel space-y-5">
      ...
      <SettingsIntegrationsPanel
        ...
        :gemini-timeout-seconds-field="geminiTimeoutSecondsField"
      />
...
const geminiTimeoutSecondsField = useNumberSettingField(
  () => localSettings.value?.gemini_provider.timeout_secs,
  (value) => { if (localSettings.value) localSettings.value.gemini_provider.timeout_secs = value },
  {
    integer: true,
    metadata: () => fieldMetadata('gemini_provider.timeout_secs'),
    enabled: () => Boolean(localSettings.value?.gemini_provider.enabled),
  },
)
