  ...
  <div class="space-y-4">
    <template v-if="mode === 'canvas'">
      <div class="grid items-start gap-4 lg:grid-cols-2">
        ...
        <FormSection
          title="Gemini Business Provider"
          subtitle="通过独立 Gemini Business 网关接入 Gemini 对话和生图，账号池与 GPT 账号池分开维护。"
        >
          <div class="settings-check-grid settings-check-grid--single">
            <div class="settings-check-item">
              <div class="settings-check-control">
                <Checkbox
                  v-model="settings.gemini_provider.enabled"
                  :disabled="fieldReadOnly('gemini_provider.enabled')"
                >
                  启用 Gemini Provider
                </Checkbox>
              </div>
          ...
          <FormField label="Gemini 网关地址">
            <Input
              v-model.trim="settings.gemini_provider.base_url"
              block
              :disabled="fieldReadOnly('gemini_provider.base_url')"
              placeholder="http://gemini-business2api:7860"
            />
          </FormField>

          <FormField label="Gemini 网关密钥">
            <Input
              v-model="settings.gemini_provider.api_key"
              type="password"
              block
              :disabled="fieldReadOnly('gemini_provider.api_key')"
              :placeholder="settings.gemini_provider.has_api_key ? '已配置，留空不修改' : '可选，按网关配置填写'"
            />
          </FormField>

          <div class="grid grid-cols-1 gap-2.5 md:grid-cols-2">
            <FormField label="对话模型（一行一个）">
              <textarea
                v-model="geminiChatModelsText"
                rows="5"
                class="ui-textarea-sm"
                :disabled="fieldReadOnly('gemini_provider.chat_models')"
                placeholder="gemini-auto&#10;gemini-2.5-pro"
              ></textarea>
            </FormField>
            <FormField label="生图模型（一行一个）">
              <textarea
                v-model="geminiImageModelsText"
                rows="5"
                class="ui-textarea-sm"
                :disabled="fieldReadOnly('gemini_provider.image_models')"
          ...
          <FormField label="请求超时（秒）">
            <SettingsNumberInput :field="geminiTimeoutSecondsField" />
          </FormField>
        </FormSection>
      </div>
    </template>
...
const props = defineProps<{
  ...
  geminiTimeoutSecondsField: NumberSettingField
}>()

const fieldReadOnly = (path: string) => settingsFieldReadOnly(props.fields, path)

const geminiChatModelsText = computed({
  get: () => settings.gemini_provider.chat_models.join('\n'),
  set: (value: string) => {
    settings.gemini_provider.chat_models = value
      .split(/\r?\n/)
      .map((item) => item.trim())
      .filter(Boolean)
  },
...
const geminiImageModelsText = computed({
  get: () => settings.gemini_provider.image_models.join('\n'),
  set: (value: string) => {
    settings.gemini_provider.image_models = value
      .split(/\r?\n/)
      .map((item) => item.trim())
      .filter(Boolean)
  },
