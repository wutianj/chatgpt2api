...
export const ACCOUNT_IMPORT_MODE_CATALOG = [
  ...
  { label: '导入 AT/RT 修复文件', value: 'credential_patch_json' },
...
export function useAccountImportRuntime(options: AccountImportRuntimeOptions) {
  ...
  const oauthTargetAccountId = ref('')
  ...
  async function finishOAuthLogin() {
    ...
    try {
      ...
      const result = await accountImportsApi.finishOAuthLogin(
        sessionId,
        callback,
        oauthTargetAccountId.value.trim(),
        targetGroupId(),
      )
  ...
  async function importLocalAccountFiles(files: FileList | File[] | null | undefined) {
    ...
    const patchingCredentials = importMode.value === 'credential_patch_json'
    let preloadedReg2Payloads: unknown[] | null = null
    let preloadedCredentialPatchPayloads: unknown[] | null = null
    if (importingReg2 || patchingCredentials) {
      try {
        const preloadedRows: unknown[] = []
        for (const file of fileList) {
          preloadedRows.push(...parseReg2AccountRows(await file.text(), file.name))
        }
        if (importingReg2) {
          preloadedReg2Payloads = preloadedRows
        } else {
          preloadedCredentialPatchPayloads = preloadedRows
        }
      } catch (error) {
        options.setError(patchingCredentials ? '导入 AT/RT 修复文件失败' : '导入 reg2 注册机账号失败', error, false)
        return
      }
      if (importingReg2 && !preloadedReg2Payloads?.length) {
        options.setError('导入 reg2 注册机账号失败', new Error('文件中没有找到 reg2 账号记录'), false)
        return
      }
      if (patchingCredentials && !preloadedCredentialPatchPayloads?.length) {
        options.setError('导入 AT/RT 修复文件失败', new Error('文件中没有找到账号凭据记录'), false)
        return
      }
    }
    ...
    const confirmed = await confirmDialog.ask({
      ...
      message: restoringBackup
        ...
        : patchingCredentials
          ? `已识别 ${preloadedCredentialPatchPayloads?.length || 0} 条凭据记录，将按 management_id / account_id / user_id / 邮箱匹配已有账号，原地替换 AT 并补 RT。是否继续？`
    ...
    try {
      ...
      const credentialPatchPayloads: unknown[] = preloadedCredentialPatchPayloads ? [...preloadedCredentialPatchPayloads] : []
      if (!importingReg2 && !patchingCredentials) {
      ...
      } else if (patchingCredentials) {
        await importCredentialPatchPayloadBatch(credentialPatchPayloads, title, true)
      } else {
  ...
  async function importCredentialPatchPayloadBatch(accountPayloads: unknown[], title: string, alreadyConfirmed = false) {
    if (!alreadyConfirmed) {
      const confirmed = await confirmDialog.ask({
        title,
        message: `即将修复 ${accountPayloads.length} 个账号凭据。只会更新已匹配的现有账号，不会新增账号。是否继续？`,
        confirmText: '确认修复',
        cancelText: '取消',
      })
      if (!confirmed) return
    ...
    const result = await accountsApi.importCredentialPatches(accountPayloads, {
      syncAfterImport: true,
      returnItems: false,
      targetGroupId: targetGroupId(),
    })
    const failed = result.errors.length
    const skippedRows = result.invalid + result.missing_credentials + result.missing_target + result.ambiguous_target
    options.bulkProgress.appendEvents(result.events || [])
    options.bulkProgress.finish({
      total: accountPayloads.length,
      processed: accountPayloads.length,
    ...
    refreshAccountListInBackground()
    if (skippedRows > 0) {
      toast.warning(`凭据修复跳过 ${skippedRows} 行：坏行 ${result.invalid}，缺凭据 ${result.missing_credentials}，未匹配 ${result.missing_target}，匹配冲突 ${result.ambiguous_target}`)
    }
  }
