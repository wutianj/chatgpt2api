...
export function useAccountsPage() {
  ...
  const oauthTargetAccountId = accountImport.oauthTargetAccountId
  ...
  async function openOAuthRelogin(item: Account) {
    const accountId = String(item.id || '').trim()
    if (!accountId) {
      toast.warning('账号 ID 缺失，不能定向更新')
      return
    }
    oauthTargetAccountId.value = accountId
    oauthEmailHint.value = item.email || item.display_name || ''
    await openImportModal('oauth_login')
