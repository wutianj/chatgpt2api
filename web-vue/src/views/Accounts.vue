    ...
    >
      ...
      >
        ...
        <AccountTableRow
              ...
              @oauth-relogin="openOAuthRelogin"
              ...
              <div class="min-h-[clamp(26rem,55vh,42rem)] p-4">
                <div v-if="importMode === 'oauth_login'" class="space-y-3">
                  ...
                  <div class="grid grid-cols-1 gap-3">
                    ...
                    <label class="block text-xs">
                      <span class="ui-field-label">目标账号 ID（可选，用于更新已有账号）</span>
                      <Input
                        :model-value="oauthTargetAccountId"
                        block
                        placeholder="acct_xxx；留空时按 OAuth 返回邮箱 / account_id 自动匹配"
                        :disabled="importBusy"
                        @update:model-value="oauthTargetAccountId = $event.trim()"
                      />
                    </label>
                ...
                <div
                  v-else-if="importMode === 'credential_patch_json'"
                ...
                >
                  <ImportModePanel
                    title="导入 AT/RT 修复文件"
                    description="读取 OAuth / Sub2API / 注册机重登后的 JSONL/JSON，按 management_id、OpenAI account_id、user_id 或邮箱匹配已有账号，只替换 AT 并补 RT，不新增账号。"
                  />
                  <StateBlock dashed compact>
                    ...
                    >
                      ...
                      <span class="account-file-dropzone__button">选择 AT/RT JSONL / JSON 文件</span>
                      <span class="account-file-dropzone__hint">不会创建新账号；未匹配记录会跳过并显示原因</span>
                    </label>
                  </StateBlock>
                </div>
